...
# The four dashboard templates, READ FROM DISK rather than built here.
#
# They used to be assembled inline, which meant this script and
# supabase/templates/*.html were two unrelated designs for the same emails -
# and running the script silently replaced the styled versions with plain ones.
# One source, generated by scripts/build_email_templates.py.
#
# Reauthentication is deliberately absent: the Management API key set below
# covers the four link-bearing templates, and that email is code-only. Paste it
# from supabase/templates/reauthentication.html in the dashboard.
TEMPLATE_DIR = Path(__file__).resolve().parent.parent / "supabase" / "templates"

TEMPLATE_FILES = {
    "confirmation": ("confirm-signup.html", "Confirm your Oreag email"),
    "magic_link": ("magic-link.html", "Your Oreag sign-in code"),
    "recovery": ("reset-password.html", "Reset your Oreag password"),
    "email_change": ("email-change.html", "Confirm your new Oreag email"),
}


def _load_templates() -> dict[str, dict[str, str]]:
    """Read each template, failing LOUDLY on a missing {{ .Token }}.

    A template without the code placeholder renders as empty space in the
    inbox: the mail arrives, looks fine, and the code field in the app has
    nothing to fill it with. That is precisely how the styled templates shipped
    broken, so it is checked here as well as in the test suite - this script can
    be run against a project long after CI last ran.
    """
    loaded: dict[str, dict[str, str]] = {}
    for key, (filename, subject) in TEMPLATE_FILES.items():
        path = TEMPLATE_DIR / filename
        if not path.is_file():
            sys.exit(f"missing template {path} - run scripts/build_email_templates.py")
        content = path.read_text(encoding="utf-8")
        if "{{ .Token }}" not in content:
            sys.exit(
                f"{filename} has no {{{{ .Token }}}} placeholder - the app asks "
                "users to type that code"
            )
        loaded[key] = {"subject": subject, "content": content}
    return loaded
