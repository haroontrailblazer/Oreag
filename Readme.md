...
https://playground.likec4.dev/w/sn1kDz/index/
...
- **Two-layer answer cache** - used by every query surface (playground, `/v1` API, MCP). L1 uses Redis (in-memory fallback, single-flight de-duplication, 1 h TTL) and matches the original request plus the conversation context before any rewrite or model call. L2 uses Postgres/pgvector (24 h TTL) for standalone questions only: similarity must reach the configured threshold (default 0.90) and the request wording must pass a conservative equivalence check. Matching a topic alone never authorizes reuse; code, detail, framework, language and other changed requirements generate fresh answers. Both layers are scoped by project, models, top-K, content version and answer policy. Responses report `cache_layer` and `cache_similarity`; similarity measures the questions, not answer quality. The request-v2 policy excludes old cache entries automatically without flushing Redis or deleting conversations.
- **Queries, Health, and answer feedback** - public API calls and Playground tests share query logs. Project API keys can list query history (`GET /v1/projects/{id}/queries`), inspect a query (`GET /queries/{query_id}` under the same project prefix), read project diagnostics (`GET /health`), and submit or remove feedback (`PUT`/`DELETE /queries/{query_id}/feedback`). Owner dashboards use the same reporting services across owned projects. One mutable helpful/not-helpful rating and an optional note are stored per query by migration 0043; feedback does not change retrieval or generated answers. See [the feedback guide](docs/answer-feedback.md) and the in-app Queries, Health, and Answer feedback documentation.
