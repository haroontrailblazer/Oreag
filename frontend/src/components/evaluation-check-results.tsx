...
export function EvaluationCheckResults({ run }: { run: EvaluationRun }) {
  return <ul className="divide-y rounded-lg border">{run.suite.cases.map(test => {
    ...
    return <li key={test.id} className="space-y-2 p-3">
      ...
      {change && <p className="text-[11px] text-muted-foreground">{change.before === change.after ? "Check result unchanged" : change.before === "failed" && change.after === "passed" ? "Now passing" : change.before === "passed" && change.after === "failed" ? "Previously passed; now failing" : "Check status changed"}</p>}
