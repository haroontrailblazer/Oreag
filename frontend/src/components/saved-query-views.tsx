...
export function SavedQueryViews<T extends Record<string, string>>({ kind, filters, onApply }: {
  ...
  async function save() {
    ...
    const payload = JSON.stringify({ name: name.trim(), kind, filters: editing?.filters ?? filters })
    if (attempt.current?.payload !== payload || (editing && attempt.current.id !== editing.id)) {
      attempt.current = { id: editing?.id ?? crypto.randomUUID(), payload }
    }
    try {
      const { id } = attempt.current
      await api(`/api/account/query-views/${id}`, { method: "PUT", body: JSON.stringify({ id, ...JSON.parse(payload) }) })
      attempt.current = null; setEditing(null); setName(""); await mutate()
  ...
  async function remove(id: string) {
    ...
    try {
      ...
      if (editing?.id === id) { setEditing(null); setName(""); attempt.current = null }
      await mutate()
    ...
    <Dialog open={open} onOpenChange={value => { if (!busy) setOpen(value) }}>
      <DialogContent className="max-h-[85dvh] overflow-y-auto sm:max-w-lg">
        ...
        <form className="space-y-3 rounded-xl border p-4" onSubmit={event => { event.preventDefault(); void save() }}>
          ...
          <div className="flex flex-wrap gap-2"><Button size="sm" type="submit" disabled={busy || !name.trim()}>{busy ? <Spinner size={14} /> : <FloppyDiskIcon aria-hidden="true" />}{editing ? "Save name" : "Save view"}</Button>{editing && <Button type="button" size="sm" variant="ghost" disabled={busy} onClick={() => { setEditing(null); setName(""); attempt.current = null }}>Cancel rename</Button>}</div>
        </form>
        ...
        {error && !isSessionExpired(error) ? <div role="alert" className="space-y-2 text-xs"><p>Could not load saved views.</p><Button variant="outline" size="sm" onClick={() => void mutate()}>Retry</Button></div>
          ...
          : <ul className="divide-y rounded-xl border">{data.map(view => <li key={view.id} className="flex min-w-0 items-center gap-1 p-2">
            ...
            <Button size="icon-sm" variant="ghost" aria-label={`Rename ${view.name}`} disabled={busy} onClick={() => { setEditing(view); setName(view.name); attempt.current = null }}><PencilSimpleIcon aria-hidden="true" /></Button>
