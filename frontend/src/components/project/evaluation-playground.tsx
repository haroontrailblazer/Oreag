...
export function EvaluationPlayground({ project, onBack }: { project: Project; onBack: () => void }) {
  ...
  const { data: history, mutate: reloadHistory } = useSWR<EvaluationRun[]>(`${base}/runs${archived ? `?archived=true&offset=${archiveOffset}` : ""}`, fetcher, { refreshInterval: 10000 })
  ...
  return <div className="relative min-w-0 space-y-5 pb-5">
    ...
    <EvaluationSchedule base={base} history={archived ? [] : history ?? []} />
    ...
    <section aria-label="Saved evaluation runs" className="space-y-3 border-t pt-4">
      <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex flex-wrap items-center gap-3"><h3 className="text-sm font-semibold">Saved runs</h3><label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={archived} disabled={running} onChange={e => { setArchived(e.target.checked); setArchiveOffset(0); setRun(null) }} />Archived</label></div><div className="w-full min-w-0 sm:w-72"><FilterSelect label="Choose a saved run" hideLabel value={run?.id ?? ""} onChange={id => { if (id && !running) void selectRun(id) }} options={[{ value: "", label: history?.length ? "Select a run" : "No runs yet" }, ...(history ?? []).map(r => ({ value: r.id, label: `${new Date(r.created_at).toLocaleString()} · ${r.status}${r.quality_report?.state === "regressed" ? " · Regression warning" : ""}` }))]} /></div></div>
      {archived && <div className="flex flex-wrap items-center justify-between gap-3 text-xs"><p>Archived results keep answers and settings. Prepared vectors are released.</p><div className="flex gap-2"><Button size="sm" variant="outline" disabled={!archiveOffset} onClick={() => setArchiveOffset(v => Math.max(0, v-20))}>Previous</Button><Button size="sm" variant="outline" disabled={!history || history.length < 20} onClick={() => setArchiveOffset(v => v+20)}>Next</Button></div></div>}
      {run && <>
        <div className="flex flex-wrap items-center justify-between gap-2"><p role="status" className="text-sm capitalize">{run.status} · {run.prepared}/{run.corpus_count * run.suite.variants.length} vectors · {run.results.length}/{run.suite.cases.length * run.suite.variants.length} answers</p><div className="flex flex-wrap gap-2">
          {!run.archived_at && !running && (active(run) || run.status === "failed") && <Button variant="outline" size="sm" onClick={() => void resume()}><PlayIcon />Resume run</Button>}
        ...
        {run.archived_at && <p className="text-xs text-muted-foreground">Archived on {new Date(run.archived_at).toLocaleString()}. Export these results or start a new comparison.</p>}
