...
export function WebhooksPanel({ projectId }: { projectId: string }) {
  ...
  return <section className="min-w-0 space-y-4 rounded-xl border bg-card p-4 sm:p-5">
    <div className="flex items-center justify-between gap-3"><div><h3 className="text-sm font-medium">Webhooks</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">Send project events to your application.</p></div><Button size="sm" variant="outline" disabled={!data || data.length >= 5} onClick={() => { setMessage(""); setOpen(true) }}>Add webhook</Button></div>
    ...
    {data?.length === 0 && <p className="py-4 text-sm text-muted-foreground">Connect a receiver for indexing, evaluations, and project budget alerts.</p>}
