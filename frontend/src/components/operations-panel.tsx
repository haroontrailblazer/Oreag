...
import { FAILURE_VIEW } from "@/lib/request-failures"
...
export function OperationsPanel({ compact = false }: { compact?: boolean }) {
  ...
  if (compact) return data?.warnings.length ? <div role="alert" className="rounded-xl border border-amber-500/30 bg-card p-3 text-xs"><p className="font-medium">Operations need attention</p><p className="mt-1 leading-5 text-muted-foreground">{data.warnings[0].message} <Link className="underline" href="/settings/usage#operations">View in Usage</Link></p>{data.errors + data.rejected > 0 && <Link href={FAILURE_VIEW} className="mt-2 inline-flex items-center gap-1 font-medium underline underline-offset-4">Explore failed requests<ArrowRightIcon aria-hidden="true" className="size-3" /></Link>}</div> : null
  return <details id="operations" className="mb-4 rounded-xl border bg-card p-4"><summary className="cursor-pointer text-sm font-medium">Operations <span className="ml-2 text-xs font-normal text-muted-foreground">{data ? data.warnings.length ? `${data.warnings.length} ${data.warnings.length === 1 ? "needs" : "need"} attention` : "Healthy" : "Measurements"}</span></summary>
    <div className="mt-4 space-y-4">
      ...
      {data && <><p className="text-xs leading-5 text-muted-foreground">Latest {data.requests.toLocaleString()} authenticated API and project requests in 24 hours{data.limited ? " (limited to 5,000)" : ""}. Measurements arrive asynchronously. Alerts use the latest 15 minutes; latency and failure alerts require 20 requests.</p>
        ...
        <Button variant="outline" size="sm" asChild><Link href={FAILURE_VIEW}>Explore failed requests<ArrowRightIcon aria-hidden="true" /></Link></Button>
