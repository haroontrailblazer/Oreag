...
export function FilterSelect({ label, value, onChange, options, hideLabel = false }: {
  ...
  return <div className="flex min-w-0 flex-col gap-1.5">
    ...
    <Select value={prefix + value} onValueChange={next => onChange(next.slice(prefix.length))}>
      <SelectTrigger id={id} title={options.find(option => option.value === value)?.label} className="w-full min-w-0 text-left [&_[data-slot=select-value]]:block [&_[data-slot=select-value]]:truncate">
        <SelectValue />
      </SelectTrigger>
