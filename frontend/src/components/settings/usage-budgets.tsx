...
import { BellIcon, PlusIcon, PencilSimpleIcon, TrashIcon } from "@phosphor-icons/react/dist/ssr"
...
export function UsageBudgets() {
  const {data, error, mutate: refresh} = useSWR<BudgetsReport>(BUDGETS_KEY, fetcher, {refreshInterval:BUDGET_REFRESH_MS})
  ...
  return <section aria-label="Budgets and alerts" className="relative mb-6 min-w-0 space-y-4 rounded-xl border bg-card p-4 sm:p-5">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><h2 className="text-sm font-semibold">Budgets &amp; alerts</h2><p className="mt-1 text-xs text-muted-foreground">{month} · USD · Warning only</p></div>
    ...
    {!data && !error && <p className="text-xs text-muted-foreground">Checking saved budgets…</p>}
    {data && !data.budgets.length && <p className="text-sm text-muted-foreground">Set a monthly budget for your account or a project. Get an early warning and an alert at 100%.</p>}
    {!!data?.budgets.length && <div className="grid gap-3 lg:grid-cols-2">{data.budgets.map(budget=><article key={budget.id} className="min-w-0 space-y-3 rounded-lg border p-3">
      ...
      <p className="text-xs text-muted-foreground">Warn at {budget.warning_percent}% and 100%. {budget.unpriced_requests>0?`${budget.unpriced_requests} events have missing prices; recorded spend may be incomplete.`:"Based on recorded model and embedding costs."}</p>
    </article>)}</div>}
