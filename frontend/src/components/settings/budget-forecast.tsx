import { CalendarBlankIcon, ChartLineUpIcon, InfoIcon } from "@phosphor-icons/react/dist/ssr"
...
export function BudgetForecast({ budget, period, asOf, refreshFailed = false }: {
  ...
  const reason = refreshFailed ? "Refresh budgets to see an updated forecast."
    : !forecast.available ? {
      incomplete: "A forecast will appear once there is enough recorded spending to estimate from.",
  ...
  return <section aria-label={`${budget.project_name ?? "Account"} budget forecast`} className="min-w-0 space-y-3 rounded-lg border border-border/60 bg-muted/20 p-3">
    ...
    {unavailable ? <div className="space-y-2">
      <p className="flex items-center gap-2 text-sm font-medium">
        {incomplete && !refreshFailed ? "Not enough data yet" : "Forecast unavailable"}
      </p>
      <p className="text-xs leading-relaxed text-muted-foreground">{reason}</p>
    </div> : forecast.available && <>
      <dl className="grid grid-cols-2 gap-3">
        <div className="min-w-0 space-y-1">
          <dt className="min-h-10 text-xs leading-5 text-muted-foreground sm:min-h-5">Month-end spend</dt>
        ...
        <div className="min-w-0 space-y-1">
          <dt className="min-h-10 text-xs leading-5 text-muted-foreground sm:min-h-5">Daily average</dt>
      ...
      {partial ? <div className="space-y-1 text-xs leading-relaxed">
        <p className="font-medium">{percent.format(forecast.projectedPercent)}% of monthly budget projected</p>
        <p className="text-muted-foreground">Based on available costs</p>
      ...
      {(!partial || forecast.alreadyReached) && <p className="flex items-start gap-2 border-t border-border/60 pt-3 text-xs leading-relaxed text-muted-foreground">
    ...
    {budget.unpriced_requests > 0 && <details className="border-t border-border/60 pt-3 text-xs leading-relaxed text-muted-foreground">
      <summary className="w-fit cursor-pointer rounded-sm font-medium focus-visible:outline-2 focus-visible:outline-offset-4">Cost details</summary>
      <p className="mt-2">{budget.unpriced_requests.toLocaleString("en-US")} {budget.unpriced_requests === 1 ? "request has" : "requests have"} no complete cost record. This can include requests that stopped before using an AI model; it does not confirm a charge. Spending and estimates include recorded costs only, so actual spending may be higher.</p>
    </details>}
  </section>
}
