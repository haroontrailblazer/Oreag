...
export function getBudgetForecast(
  ...
  if (budget.spent_usd == null || !Number.isFinite(budget.spent_usd) || budget.spent_usd < 0 || budget.unpriced_requests > 0) {
  ...
  const reachesBudgetAt = !alreadyReached && difference > 0 && reachTime != null && reachTime >= now && reachTime < end
    ? new Date(reachTime).toISOString() : null
  return { available: true, elapsedDays, monthDays, dailyAverage, projectedSpend,
